...
Tale processo di migrazione non si è limitato alla semplice riscrittura del codice esistente, ma ha comportato una revisione approfondita dell'architettura sottostante, con l'obiettivo di superare i limiti riscontrati nella soluzione originaria. In particolare, il lavoro svolto ha permesso di conseguire un netto miglioramento sotto tre aspetti fondamentali: l'affidabilità del sistema, intesa come capacità di garantire un funzionamento stabile e resiliente anche in condizioni di carico elevato; l'efficienza operativa, in termini di ottimizzazione delle risorse computazionali e riduzione dei tempi di risposta; e infine l'usabilità, sia dal punto di vista degli utenti finali sia da quello degli sviluppatori chiamati a mantenere ed estendere il sistema nel tempo.
...
\begin{description}
    ...
    \item[\Cref{chap:deployment} -- Deployment e integrazione:] approfondisce le strategie e le decisioni operative adottate per il dispiegamento (\emph{deployment}) e l'integrazione delle nuove componenti all'interno dell'infrastruttura di rete e del nuovo applicativo.
    
    \item[\Cref{chap:results} -- Validazione e test:] presenta le prove sperimentali e i test condotti sul campo al fine di verificare la correttezza, la robustezza e le prestazioni dei componenti realizzati.
