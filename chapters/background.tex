...
La \cref{fig:legacy-architecture} illustra schematicamente l'architettura delle tre componenti; di seguito ciascuna di esse viene descritta in dettaglio.

\begin{figure}
    \centering
    \includegraphics[width=1\linewidth]{figures/legacy-architecture.pdf}
    \caption[Diagramma dell'Architettura del servizio legacy.]{In ogni firewall della rete sono presenti uno o più servizi di downstream.}
...
\begin{itemize}
    \item \emph{Discovery dei server attivi}: recupero dinamico della lista dei server della rete da interrogare ai fini della misurazione della latenza, garantendo che solo i nodi effettivamente operativi vengano coinvolti nel processo di raccolta;
    \item \emph{Distribuzione delle richieste}: inoltro, tramite chiamate \ac{HTTP}, delle richieste di misurazione verso ciascun nodo Downstream individuato nella fase precedente;
    \item \emph{Aggregazione dei risultati}: raccolta delle risposte provenienti dai singoli nodi Downstream, effettuando l'unione dei dati parziali in un'unica struttura coerente;
    \item \emph{Riordinamento}: riordinamento dei risultati aggregati per valore di latenza crescente, in modo da restituire un output immediatamente consultabile dal componente che effettua la richiesta finale.
\end{itemize}
...
\paragraph{Controller PhP.} Il primo elemento è un controller scritto in PhP, responsabile della gestione delle richieste in ingresso provenienti dai client (nello specifico, dalla route PhP di FlashStart Cloud descritta nella Sezione~\ref{section:route-php}).

Il controller si occupa della validazione dei parametri ricevuti, dell'invocazione dello script Python descritto di seguito e della formattazione della risposta finale da restituire al chiamante.
...
\paragraph{Script Python.} Il secondo elemento è uno script Python esterno, invocato dal controller PhP come processo separato, a cui è delegata la logica core dell'orchestrazione. In particolare, lo script è responsabile di eseguire le funzioni descritte in precedenza.
...
A differenza di quest'ultimo, il Downstream non è un componente unico, bensì viene replicato e distribuito su ciascuno dei \ac{DNS} resolver appartenenti alla rete anycast del servizio.
...
\lstinputlisting[
    language=c++,
    label={lst:ping-output},
    caption={[Output del comando ping.] Esempio dell'output del comando ping atteso dal parser.}
    ]{listings/Ping-example}
...
Il servizio di downstream è integrato all'interno del \ac{DNS} provider; Per questo motivo in ciascun server della rete anycast ci potrebbero essere diversi dispiegamenti del servizio, consumando inutilmente risorse computazionali importanti.
...
\lstinputlisting[
    float,
    language=php,
    label={lst:route-inefficiencies},
    caption={[Implementazione legacy della route PhP.] Per ciascun elemento ritornato dall'upstream aggregator viene aperta una connessione al database ed eseguite due query complesse influendo notevolmente sul tempo di esecuzione del servizio.}
    ]{listings/route.php}
Lo script contiene alcune inefficienze che impattano sulle prestazioni generali del servizio.
\begin{itemize}
    \item \emph{Filtraggio delle risposte}: lo script include una funzione per filtrare le risposte duplicate, dovute alla presenza di più resolver \ac{DNS} sullo stesso server fisico.
    \item \emph{Operazioni onerose}: per ciascun risultato vengono eseguite diverse operazioni pesanti, riportate nel \cref{lst:route-inefficiencies}, molte delle quali si rivelano inutili a causa del successivo filtraggio dei duplicati.
    \item \emph{Ordinamento ridondante}: il risultato ottenuto viene nuovamente ordinato, nonostante fosse stato già eseguito dalla componente di upstream.
\end{itemize}
...
Complessivamente, il flusso di una richiesta attraversa le tre componenti descritte secondo la seguente sequenza e illustrato in \cref{fig:information-flow}:
...
\begin{enumerate}
    ...
    \item l'Upstream Orchestrator recupera la lista dei server attivi e distribuisce, tramite chiamate \ac{HTTP}, le richieste di misurazione ai relativi componenti Downstream dislocati sulla rete anycast;
...
\begin{figure}
    \centering
    \includegraphics[width=1\linewidth]{figures/information-flow.pdf}
    \caption[Diagramma di sequenza del latency service.]{Flusso delle informazioni partendo dalla richiesta del client fino alla visualizzazione dell'informazione ottenuta sull'interfaccia.}
