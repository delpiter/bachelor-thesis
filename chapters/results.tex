...
Come descritto in \cref{sec:variable-load} sia il servizio di upstream che quello di downstream necessitano del caricamento di un insieme di variabili d'ambiente indispensabili per il corretto funzionamento del sistema.
...
Per la validazione delle route \ac{HTTP} esposte dai due servizi è stato utilizzato il package \texttt{net/http/httptest}, appartenente alla standard library di Go, che consente di simulare richieste \ac{HTTP} senza la necessità di avviare un server reale e in ascolto su una porta di rete.

Attraverso questo strumento sono state simulate le chiamate \texttt{GET} verso gli endpoint esposti da upstream e downstream, permettendo di verificare:
\begin{itemize}
...
Questa modifica ha come conseguenza due miglioramenti: minore consumo di memoria in una singola macchina dato da inutili copie del servizio e maggiore efficienza nel processo di aggregazione, in quanto il numero di server da interrogare è notevolmente ridotto.
Viene inoltre rimossa la necessità della funzione di filtraggio degli indirizzi duplicati, descritta in \cref{section:route-php}, in quanto ogni macchina viene interrogata una volta sola.